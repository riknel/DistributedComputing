...
//обычный алгоритм однопоточного merge
...
//такой же алгоритм многопоточного слития, только последовательный код
void merge_non_parallel(int* array, int* copy_array, int left_1, int right_1, int left_2, int right_2, int left_3, int m){
    ...
    merge_non_parallel(array, copy_array, left_1, middle_1 - 1, left_2, middle_2 - 1, left_3, m);
    merge_non_parallel(array, copy_array, middle_1 + 1, right_1, middle_2, right_2, middle_3 + 1, m);
}

//такой же алгоритм многопоточной сортировки, только последовательный код
void sort_non_parallel(int* array, int* copy_array, int left, int right, int m) {
    int n  = right - left + 1;
    ...
    if(n < m) {
        ...
        sort_non_parallel(array, copy_array, left, middle, m);
        sort_non_parallel(array, copy_array, middle + 1, right, m);
        merge_non_parallel(array, copy_array, left, middle, middle + 1, right, left, m);
...
int main(int argc, char **argv){
  ...
  struct timeval begin, end;
	...
	FILE *f_stats = fopen("stats_for_graphs.txt", "a");
  ...
  int* test_q = (int*)malloc(sizeof(int) * n);
  assert(gettimeofday(&begin, NULL) == 0);
  memcpy(test_q, array, sizeof(int) * n);
  qsort(test_q, n, sizeof(int), comp);
  assert(gettimeofday(&end, NULL) == 0);
  double time_qsort = ((end.tv_sec - begin.tv_sec) * 1000000u + end.tv_usec - begin.tv_usec) / 1.e6;

  assert(gettimeofday(&begin, NULL) == 0);
  sort_non_parallel(array, copy_array, 0, n - 1, m);
  assert(gettimeofday(&end, NULL) == 0);
  double program_time1th = ((end.tv_sec - begin.tv_sec) * 1000000u + end.tv_usec - begin.tv_usec) / 1.e6;
  ...
  for(int p = 1; p <= 16; ++p) {
    ...
    double S_p = program_time1th / program_time;
    double E_p = S_p / p;
    fprintf(f_stats, "%d %f %f %f %f\n", p, program_time, S_p, E_p, time_qsort);
  }
